...
def train():
    ...
    #     original_wpe_weight = model.transformer.wpe.weight
    #     original_length = original_wpe_weight.shape[0]
    #     new_length = training_args.model_max_length
        
    #     new_positions = torch.linspace(0, original_length-1, new_length)
    #     new_wpe_weight = torch.nn.functional.interpolate(original_wpe_weight.permute(1, 0).unsqueeze(0), size=new_length, mode='linear', align_corners=False).squeeze(0).permute(1, 0)
    #     # new_wpe_weight = torch.nn.functional.interpolate(original_wpe_weight.unsqueeze(0), size=new_length, mode='linear', align_corners=False).squeeze(0)
    #     model.transformer.wpe.weight = torch.nn.Parameter(new_wpe_weight)
    ...
    for split_name in ["train", "validation", "test"]:
        file_path = os.path.join('./data/pg19_4096', f"{split_name}.jsonl")
    ...
    numpy_arrays = [tensor.to(torch.float32).cpu().numpy()[np.newaxis, :] for tensor in trace_gpt_attn]
    
    stacked_array = np.vstack(numpy_arrays)
    target_path = os.path.join(training_args.output_dir, "attn.npy")
    np.save(target_path, stacked_array)
