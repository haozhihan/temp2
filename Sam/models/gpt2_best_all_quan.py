...
from Sam.utils.profile_utils import h_mask,quant_this,quant
from Sam.utils.others import pack_hook, unpack_hook
import torch
...
class GPT2Attention(nn.Module):
    ...
    def _attn(self, query, key, value, attention_mask=None, head_mask=None):
        attn_weights = torch.matmul(query, key.transpose(-1, -2)) #[25, 2048, 64],[25, 64, 2048] bf16
        ...
        with torch.no_grad():
            # global quant_this
            quant_this.append(quant[self.layer_idx])
        if self.training:
            with torch.autograd.graph.saved_tensors_hooks(pack_hook, unpack_hook):
                attn_weights = nn.functional.softmax(attn_weights, dim=-1)   #[1,25, 2048, 2048] float32
        else:
            attn_weights = nn.functional.softmax(attn_weights, dim=-1)

        # Downcast (if necessary) back to V's dtype (if in mixed-precision) -- No-Op otherwise
        attn_weights = attn_weights.type(value.dtype)
        attn_weights = self.attn_dropout(attn_weights) #[1, 25, 2048, 2048] bool

        # Mask heads if we want to
        if head_mask is not None:
            attn_weights = attn_weights * head_mask #[1, 25, 2048, 2048] bool
        # print(self.layer_idx)
        if self.training:
            with torch.autograd.graph.saved_tensors_hooks(pack_hook, unpack_hook):
                attn_output = torch.matmul(attn_weights, value) #[25, 2048, 2048],[25, 2048, 64] bf16
        else:
    ...
    def forward(
        ...
        if self.training:
            with torch.no_grad():
                hh_mask = h_mask[self.layer_idx]
                # print(hh_mask[0,0])
                block_size = query.shape[2] // 16
