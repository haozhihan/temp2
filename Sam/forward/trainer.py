...
# isort: on
from Sam.utils.profile_utils import h_mask, quant
from Sam.utils.others import select_positions_based_on_sum
...
class Trainer:
    ...
    def __init__(
        self,
        ini_model: Union[PreTrainedModel, nn.Module] = None,
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        with torch.no_grad():
            a=0
            t=0
            for k,v in h_mask.items():
                a+= v.sum()/25
                t+=1
                # print(v.shape)
            # if t>0:
            #     print(f'ceng:{t},,average:{a/t}')
            h_mask.clear()
            # if a==0:
            _ = self.ini_model(**inputs)
            # for k,v in h_mask.items():
            #     vv = v[0].sum(dim=0)
            #     quant[k] = select_positions_based_on_sum(vv)
            #     # print(f'{k}:{quant[k]}')
